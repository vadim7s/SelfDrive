...
# start from map_grab_continue.py

# add weather - added 2 args:       '-i', '--img_count'  - total image count expected from this town
#                                   '-w', '--weather_count' - minimup images for one weather changes - MUST be smaller than total above
# add expected number of images - done - see param above
# add image synching after


# problem remaining: often speed is falsely read as zero and images are not written
# fix needed - change image generation criteria from speed to min distance from last image
...
Use ARROWS or WASD keys for control.
    ...
    Space        : hand-brake
    P            : toggle autopilot
    M            : DISABLED toggle map recording on/off (only works when map is chosen - key 2)
    ...
    R            : toggle recording images and map captures to disk 
...
# mounting point on Model 3 to achieve close to real car image 
CAMERA_POS_Z = 1.3 
CAMERA_POS_X = 1.4 
...
# ==============================================================================
# -- Util copied from "no_rendereing.." example---------------------------------
# ==============================================================================
...
class World(object):
    def __init__(self, carla_world, hud, args):
        ...
        self.finish = False #end indicator to force end when image generation is finished
        try:
        ...
        # mapping attributes from no_rendering
        ...
        self.hero_previous_transform = None
        ...
        self.route = None # a route to follow if switch to 'follow route'
        # Map info
        self.map_image = None
    ...
    def restart(self):
        ...
        self.distance_from_last_iamge  = 0.0
        self.distance_from_last_iamge = 0.0 # to enable taking image decsions only when moved enough from last location when image was taken
        # Keep same camera config if the camera manager exists.
        cam_index = self.camera_manager.index if self.camera_manager is not None else 0
        cam_pos_index = self.camera_manager.transform_index if self.camera_manager is not None else 0
        # Get a Tesla Model 3 blueprint.
        blueprint = self.world.get_blueprint_library().filter('*model3*')[0]
        # old version: blueprint = random.choice(get_actor_blueprints(self.world, self._actor_filter, self._actor_generation))
        ...
        self.player.set_attribute("image_count", 0)
        self.player.set_attribute("weather_count",0)
        self.player.set_attribute("previous_transform", self.player.get_transform()) 
        
        
        # mapping set-up - copied from no_rendering
        # Create Surfaces
    ...
    def select_random_route(self):
        '''
        retruns a random route for the car from its current location
        to possible locations (spawn points) 
        where distance is longer than 100 waypoints
        '''    
        point_a = self.player.get_transform().location #we start at where the car is or last waypoint
        sampling_resolution = 1
        grp = GlobalRoutePlanner(self.map, sampling_resolution)
        # now let' pick the longest possible route
        min_distance = 100
        route_list = []
        for loc in self.map.get_spawn_points(): # we start trying all spawn points 
    ...
    def tick(self, clock,args):
        ...
        if self.player is not None:
            ...
            if self.hero_previous_transform is None:
                self.hero_previous_transform = self.hero_transform
        self.distance_from_last_iamge = self.hero_transform.location.distance(self.hero_previous_transform.location)
    ...
    def map_render(self, display):
        ...
        #pygame.image.save(rotated_result_surface, DEBUG_FOLDER+'%06d.png' % self.hud.time_grab)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    world.map_capture = True
                elif event.key == K_r and (pygame.key.get_mods() & KMOD_CTRL):
                    if (world.recording_enabled):
                        client.stop_recorder()
                        world.recording_enabled = False
                        world.map_capture = False
...
class HUD(object):
    def __init__(self, args):
        ...
        self._notifications = FadingText(font, (args.width, 40), (0, args.height - 40))
        self.help = HelpText(pygame.font.Font(mono, 16), args.width, args.height)
        self.server_fps = 0
        self.frame = 0
    ...
    def tick(self, world, clock,args):
        ...
        self._info_text = [
            ...
            'Total Images: (%5.0f)' % (world.camera_manager.images_saved_count),
            'Weather Images: (%5.0f)' % (world.camera_manager.images_weather_count),
        ...
        if world.camera_manager._parent.get_attribute("image_count") >= args.img_count:
            world.finish = True
        if world.camera_manager._parent.get_attribute("weather_count") >= args.weather_count:
            world.next_weather()
            world.camera_manager._parent.set_attribute("weather_count",0)
...
class CameraManager(object):
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        # set to zero image count to keep track of
        self.images_saved_count = 0
        self.images_weather_count = 0 # for changing weather tracking
    ...
    def set_sensor(self, index, notify=True, force_respawn=False):
        ...
        if needs_respawn:
            ...
            # circular reference.
            weak_self = weakref.ref(self)
            self.sensor.listen(lambda image: CameraManager._parse_image(weak_self, image,self.world))
        if notify:
    ...
    @staticmethod
    def _parse_image(weak_self, image, wld):
        self = weak_self()
        if not self:
            return
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            array = array[:, :, ::-1]
            self.surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
        prior_loc = self._parent.get_attribute("previous_transform")
        current_loc = self._parent.get_transform()
        if self.recording  and prior_loc.location.distance(self._parent.get_transform().location)> MIN_MOVE_BETWEEN_IMAGES:
            image.save_to_disk(IMG_FOLDER+'%08d' % self.hud.time_grab)
            img_count = self._parent.get_attribute("image_count")
            wth_count = self._parent.get_attribute("weather_count")
            self._parent.set_attribute("image_count",img_count+1)
            self._parent.set_attribute("weather_count",wth_count+1)
            self.player.set_attribute("previous_transform", current_loc) 
...
def game_loop(args):
    ...
    try:
        ...
        if args.autopilot and not sim_world.get_settings().synchronous_mode:
            print("WARNING: You are currently in asynchronous mode and could "
                  "experience some issues with the traffic simulation")
        # this is main display from original example manual_control.py
        ...
        while True:
            ...
            if controller.parse_events(client, world, clock, args.sync) or world.finish:
                return
        ...
        pygame.quit()
        synchronize_folders(MAP_FOLDER, IMG_FOLDER) # synch resulting images and maps, then re-scale maps to become same size as images
...
def main():
    ...
    argparser.add_argument(
        ...
        default=500,
        type=int,
        help='number of images to generate for one town (default: 500)')
    argparser.add_argument(
        '-w', '--weather_count',
        default=100,
        type=int,
        help='number of images to generate for each weather (default: 100)')
    argparser.add_argument(
        '--res',
