...
for request: I have training examples of RGB images where input is RGB image of 640x480 pixels 
and output is RGB image of 240x264 pixels. Those are front facing camera images and corresponding top down images of a map. 
I would like to train a U-net in pytorch using those pairs to be able to generate a map from input images. 
Please provide python code and also use image pre-processing with yield function to limit the use of RAM during training

Later added saving last image prediction at the end of each epoch to show it the training is going in the right direction
'''
import os
import numpy as np
...
def save_predicted_image(tensor_in,tensor_out, epoch, output_dir='C://SelfDrive//2025 Map from vision//prediction_eg'):
    ...
    # Save as PNG
    img_in.save(os.path.join(output_dir, f"epoch_{epoch+1}_train.png"))
    img_out.save(os.path.join(output_dir, f"epoch_{epoch+1}_predicted.png"))
...
target_dir = 'C://SelfDrive//2025 Map from vision//debug'

# Dataset and DataLoader
dataset = MapDataset(input_dir, target_dir, transform=transform)
...
for epoch in range(num_epochs):
    ...
    save_predicted_image(inputs[0],outputs[0], epoch)
    # update stats
    print(f'Epoch [{epoch+1}/{num_epochs}], Loss: {epoch_loss/len(dataloader):.4f}')
torch.save(model.state_dict(), 'C://SelfDrive//2025 Map from vision//unet_model_20250112.pth')