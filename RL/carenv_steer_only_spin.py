'''
steer only RL with a spin after spawn to prevent cheating by driving straight
also only cropped image of the road is given
...
class CarEnv(gym.Env):
	...
	def __init__(self):
		...
		self.observation_space = spaces.Box(low=0.0, high=1.0,
                                            shape=(self.new_height, self.new_width, N_CHANNELS), dtype=np.float32)
	...
	def step(self, action):
		...
		if self.episode_start + SECONDS_PER_EPISODE < time.time():
			...
			self.cleanup()
		self.cropped_image = self.front_camera[self.height_from:,self.width_from:self.width_to]
		return self.cropped_image/255.0, reward, done, {}	#curly brackets - empty dictionary required by SB3 format
	...
	def reset(self):
		...
		return self.cropped_image/255.0
