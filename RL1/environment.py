...
Outcome: it can - success
Improvement: slow speed is only punished at the start of episode
...
class CarEnv(gym.Env):
	...
	def step(self, action):
		...
		# and reward for speed in 15-30 kmh range
		v = self.vehicle.get_velocity()
		kmh = int(3.6 * math.sqrt(v.x**2 + v.y**2 + v.z**2))
		if kmh < 0.1 and self.step_counter<5:
			reward = reward - 5
		elif kmh < 0.1 and self.step_counter<10:
			reward = reward - 5
		elif kmh < 0.1 and self.step_counter<20:
			reward = reward - 10
		elif kmh < 0.1 and self.step_counter<30:
			reward = reward - 20
		elif kmh < 0.1 and self.step_counter<50:
			reward = reward - 50
		elif kmh > 15 and kmh< 30:
			reward = reward + 5
		elif kmh > 60:
			reward = reward - 20
