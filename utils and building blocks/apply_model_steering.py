...
class KeyboardControl(object):
    ...
    def __init__(self, world, start_in_autopilot, model):
        self.img_width = 640
        self.img_hight = 360
        self.height_req_portion = 0.4 #bottom share, e.g. 0.1 is take lowest 10% of rows
        self.width_req_portion = 0.5
        self.height_from = int(self.img_hight * (1 -self.height_req_portion))
        self.width_from = int((self.img_width - self.img_width * self.width_req_portion) / 2)
        self.width_to = self.width_from + int(self.width_req_portion * self.img_width)

        self._autopilot_enabled = start_in_autopilot
        self._ackermann_enabled = False
        self._ackermann_reverse = 1
        self._steer_model = model
        if isinstance(world.player, carla.Vehicle):
            self._control = carla.VehicleControl()
            self.test_model_mode = False
    ...
    def parse_events(self, client, world, clock):
            ...
            if event.type == pygame.QUIT:
                ...
                if isinstance(self._control, carla.VehicleControl):
                    ...
                    if event.key == K_q:
                        ...
                        self.test_model_mode = not self.test_model_mode
                        world.hud.notification('%s Test Model Mode' %
                                               ('Model Steering' if self.test_model_mode else 'Auto'))
        ...
        if not self._autopilot_enabled:
            if isinstance(self._control, carla.VehicleControl):
                if self.test_model_mode:
                    #implement model's steering
                    image = world.camera_manager.img_bw
                    image = cv2.resize(image, (self.img_width,self.img_hight))
                    # this version adds taking lower side of the image
                    image = image[self.height_from:,self.width_from:self.width_to]
                    image_final = image.reshape(-1, image.shape[0], image.shape[1], 1) 
...
class CameraManager(object):
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        self.transform_index = 0 #set to custom like for model image generation
        self.sensors = [
    ...
    def render(self, display):
        if self.surface is not None:
            ...
            # this is to capture image for steering model
            view = pygame.surfarray.array3d(display)
            #  convert from (width, height, channel) to (height, width, channel)
            view = view.transpose([1, 0, 2])
            #  convert from rgb to gray
            self.img_bw = cv2.cvtColor(view, cv2.COLOR_RGB2GRAY)
            #now we are ready to run .predict on the image 
...
def game_loop(args):
    ...
    try:
        ...
        controller = KeyboardControl(world, args.autopilot, model)
